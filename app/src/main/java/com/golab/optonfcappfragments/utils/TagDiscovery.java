...
/**
 * Class creating a Listenable Future performing the discovery of a Tag.
 * Was updated due to Asynchronous Task being deprecated.
 *
...
public class TagDiscovery {
    ...
    /**
     * Method to start tag discovery on an instance of TagDiscovery. On a successful tag discovery
     * onSuccess is triggered by a ListenableFuture.
     *
     * IMPORTANT: This function will fail if it's called from the UI Thread.
     *
     * @param tag - A newly discovered NFC tag.
     * @return STException - The tag type.
     */
    public STException discoverTag(Tag tag) {
        Callable<STException> call = (() -> {
            mAndroidTag = tag;
            TagInfo tagInfo;
            try {
        ...
        ListenableFuture<STException> future = mExecutor.submit(call);

        Futures.addCallback(future, new FutureCallback<STException>() {
            @Override
            public void onSuccess(STException result) {
    ...
    /**
     * Constructor of TagDiscovery class.
